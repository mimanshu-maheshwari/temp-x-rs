...
async fn main() -> Result<()> {
    ...
    let (model, mut tokenizer) = load_model(&model_path, &config_path, &tokenizer_path, &device).await?;
    // let model = candle_core::safetensors::load(model_path, &device).unwrap();
...
async fn download_model_files() -> Result<(PathBuf, PathBuf, PathBuf)> {
    ...
    println!("{:?},{:?},{:?}",model_path, tokenizer_path, config_path);
